...
default mode, called ``view'' mode, is to open the best document found in an
appropriate viewer. It is very handy for quick access to a precise document.
On the other hand, there may be other relevant documents for the given
©«name»©, which are ignored in view mode.
...
Usually, texdoc shows you only results it considers relevant. If there are no
``good'' results, it falls back to less relevant results. You can force
texdoc to show you also ``bad'' results even when there are good ones by
using the ``showall'' mode.
...
If you always (or mostly) use the same mode, you don't want to keep type the
...
Texdoc use various configuration files, which you can see using the ©-f© or
...
how texdoc proceeds to find the best results.
...
bad score since they are almost never documentation.\footnote{They often end
  ...
  directory as the documentation itself in \texlive. Other source files are
...
complicated to guess.\footnote{But not totally impossible: using the \texlive
  database, texdoc can know which documentation files are associated with
...
texdoc to ask you to choose a file to view when there are multiple choices,
